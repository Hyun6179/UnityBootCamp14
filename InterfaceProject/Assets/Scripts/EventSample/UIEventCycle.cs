    ...
    IMoveHandler                // 키보드나 조이스틱 관련
{
    ...
    private void Handle(string eventName, BaseEventData eventData)
    {
        ...
        float delta = now - lastEventTime; // 직전 이벤트와의 시간 간격을 계산합니다.
        lastEventTime = now;
        ...
        sb.Append($" <color=blue>{delta : F3}s</color>"); // 이벤트 시간 간격
        // F3 : Fixed-point(소수점 고점) 형태로 소수점 이하 3자리까지 표현하시오.
