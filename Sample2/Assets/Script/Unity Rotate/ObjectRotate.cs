...
// 쿼터니언으로 처리하는 이유
// 유니티에서 쿼터 -> 오일러 각 변환시 360도 이상의 회전은 보정될 수 있음.
// 380도 회전 -> 20도 회전

// 짐벌 락 현상(Gimbol Lock) : 오일러 각을 이용해 회전을 표현하는 경우에 발생하는 회전 자유도의 손실
...
public class ObjectRotate : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // transform.Rotete(Vector3 axis,float angle) // 특정 축을 중심을 회전
        // transform.Rotate(Vector3 axis, float angle, Space space); // 월드 또는 로컬중에 선택

        // 월드 기준으로 Z축으로 60도 만큼 회전해라 
