...
public class RayCastScene : MonoBehaviour
{
    ...
    // float maxDistance, int layerMas);
    ...
    private void Start()
    {
            ...
            // 반복문으로 콜라이더 체크
            for (int i = 0; i<hits.Length;i++) // hits 길이(개수)만큼 반복을 진행
            {
    ...
    void Update()
    {
       ...
       //{
        if (Physics.Raycast(transform.position, transform.forward,out hit, length, layerMask))
            ...
            // ~에 의해 작성된 ~(1<<n)은 해당 레이어를 제와한 모든 레이어를 의미합니다. 
