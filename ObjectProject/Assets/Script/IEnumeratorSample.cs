using UnityEngine;
using System.Collections; // 묶음을 관리하는 인터페이스
...
public class IEnumeratorSample : MonoBehaviour
{
    ...
    void Start()
    {
        // numbers 라는 배열을 순회할 수 있는 IEnumberator 형태의 데이터로 변환합니다.(C#)
        IEnumerator enumerator = numbers.GetEnumerator();

        while (enumerator.MoveNext()) // MoveNext()의 예제로 볼 수 있음.
        {
        ...
        // foreach는 순환 가능한 데이터를 순차적으로 작업할 때 사용하는 for문임으로 궁합이 잘 맞는다.
    ...
    // yield return을 사용하는 메소드는 IEnumberable 또는 IEnumerator 인터페이스를 구현하게 됩니다.
    ...
    // IEnumerable : 반복 가능한 컬렉션을 나타내는 인터페이스 입니다.
    ...
    //               MoveNext()를 통해서 다음 값을 접근  ㄱ
    //               Current를 통해서 현재 값 확인        ㅏ 기본적으로 제공됨
