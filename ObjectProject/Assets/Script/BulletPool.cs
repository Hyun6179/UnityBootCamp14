...
// 필요할 때 활성화하고 사용하지 않을 때 비활성화하는 식으로 재사용을 진행하는 방식
// 설계 패턴


// 사용 목적
// 탄알, 이펙트, 데미지 텍스트, 몬스터 처럼 자주 생성되고 사라지는 값들을
// 매번 new, destroy를 통해 생성 삭제가 발생하면 GC가 많이 호출되게 되고 이는
// 성능 저하로 이어질 수 있기에 성능 향상을 목적으로 사용합니다.
...
public class BulletPool : MonoBehaviour
{
    ...
    // 풀로 자주 사용되는 자료구조
    // 1. 리스트(List) : 데이터를 순차적으로 저장하고, 추가, 삭제가 자유롭기 때문에 효과적 (LIFO)
    // 2. 큐(Queue) : 데이터가 들어온 순서대로 데이터가 빠져나가는 형태의 자료구조 입니다.(FIFO)
    ...
    {
        // 비활성화 되어있는 총알을 찾아 활성화 합니다.
        foreach (var bullet in pool)
        {
            //계층 창에서 활성화가 안되어 있다면 (사용하지 않고 있다면)
